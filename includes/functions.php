...
/**
 * Check if auto-assign is enabled for a specific project stage and assign to admin if needed
 *
 * @param int $project_id The project ID
 * @param string $stage The project stage ('wp_conversion' or 'golive')
 * @return bool True if assignment was made, false otherwise
 */
function checkAndAutoAssignToAdmin($project_id, $stage) {
    global $conn;
    ...
    // Check if auto-assign setting is enabled for this stage
    $setting_key = "auto_assign_" . $stage;
    $setting_stmt = $conn->prepare("SELECT setting_value FROM settings WHERE setting_key = ?");
    ...
    $setting_enabled = (bool)$setting_result->fetch_assoc()['setting_value'];
    ...
    // Check if project is already assigned to someone
    $check_stmt = $conn->prepare("SELECT id, qa_user_id FROM qa_assignments WHERE project_id = ?");
    ...
    try {
        if ($assignment_result->num_rows > 0) {
            ...
            // Otherwise, update assignment to admin
            ...
            $insert_stmt = $conn->prepare("INSERT INTO qa_assignments (project_id, qa_user_id, assigned_by, assigned_at) VALUES (?, ?, ?, NOW())");
            $insert_stmt->bind_param("iii", $project_id, $admin_id, $admin_id);
            $insert_stmt->execute();
        }

        // Add to project status history
        $action = "Auto-assigned to admin for " . ($stage == 'wp_conversion' ? 'WP Conversion QA' : 'Go-Live QA');
        ...
        $project_name = $project_stmt->get_result()->fetch_assoc()['name'];

        // Create notification for admin
        $notification_content = "You have been automatically assigned to QA project: " . $project_name . " (" . ($stage == 'wp_conversion' ? 'WP Conversion' : 'Go-Live') . " stage)";
        $notification_stmt = $conn->prepare("INSERT INTO notifications (user_id, type, message, created_at, is_read) VALUES (?, 'info', ?, NOW(), 0)");
        $notification_stmt->bind_param("is", $admin_id, $notification_content);
        ...
        $conn->rollback();
        error_log("Error in auto-assign: " . $e->getMessage());
        return false;
    }
}
